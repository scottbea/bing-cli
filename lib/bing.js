...
var bingSearchCLI = function() {
    ...
    if (!error && response.statusCode == 200) {
      ...
      if (url) {
        ...
        opt.fallback = function() {
           ...
           imageToAscii(url, {size: {width: widthPercentage}, px_size: {width: 1, height: 1}}, (err, converted) => { console.log(err || converted); });
        };
  ...
  function NewsCallback(error, response, body) {
    if (!error && response.statusCode == 200) {
      console.log();
      var info = JSON.parse(body);
      console.log(("News: " + (_category ? _category : _q)).bold);
